    ...
    if args:
        if args[0] == "json":
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "fallen_fbanned_users.json"
                update.effective_message.reply_document(
                    document=output,
                    filename="fallen_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "fallen_fbanned_users.csv"
                update.effective_message.reply_document(
                    document=output,
                    filename="fallen_fbanned_users.csv",
...
def fed_user_help(update: Update, context: CallbackContext):
 ...
 • `/fednotif <on/off>`*:* Federation settings not in PM when there are users who are fbanned/unfbanned
