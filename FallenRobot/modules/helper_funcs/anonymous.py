...
from .decorators import fallencallback
        ...
        def awrapper(update: Update, context: CallbackContext, *args, **kwargs):
            ...
            if is_anon:
                ...
                if getattr(mem, permission.value) is True or mem.status == "creator" or user_id in DRAGONS:
                    ...
                    return message.reply_text(f"You don't have permissions to: `{permission.name}`",
...
def anon_callback_handler1(upd: Update, _: CallbackContext):
    ...
    if mem.status not in [ChatStatus.ADMIN.value, ChatStatus.CREATOR.value]:
        callback.answer("You aren't admin.")
        ...
        callback.answer("This isn't for you baby")
