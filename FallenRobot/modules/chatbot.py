...
async def hmm(client, message):
    ...
    if chat_id in en_chats:
        ...
        response = response.replace("Miles", "FallenRobot")
        response = response.replace("Cutiepii", "FallenRobot")
        response = response.replace("Luna", "Scenario")
        response = response.replace("Luna", "Scenario")
        pro = response
        try:
            await fallen.send_chat_action(message.chat.id, "typing")
        ...
        test = test.replace("Luna", "FallenRobot")
        test = test.replace("Luna", "FallenRobot")
        response = await lunaQuery(
            test, message.from_user.id if message.from_user else 0
        )
        response = response.replace("aco", "FallenRobot")
        response = response.replace("Aco", "FallenRobot")
        response = response.replace("Luna", "FallenRobot")
        response = response.replace("Luna", "FallenRobot")
        pro = response
        if not "en" in lan and not lan == "":
            try:
