...
from FallenRobot.utils.mongo import is_user, add_user, remove_user
...
async def gen_token(_, message: Message):
    ...
    if not await is_user(message.from_user.id):
        ...
        await message.reply_text(f"ʜᴇʀᴇ ɪs ʏᴏᴜʀ ᴛᴏᴋᴇɴ ғᴏʀ ғᴀʟʟᴇɴ ᴄʜᴀᴛʙᴏᴛ ᴀᴘɪ :\n<code>{api}</code>\n\n<b>ᴅᴏɴ'ᴛ sʜᴀʀᴇ ᴛʜɪs ᴛᴏᴋᴇɴ ᴡɪᴛʜ ᴀɴʏᴏɴᴇ ᴇʟsᴇ.</b>")
    else:
        fallen = await is_user(message.from_user.id)
        await message.reply_text(f"ʏᴏᴜ'ᴠᴇ ᴀʟʀᴇᴀᴅʏ ɢᴇɴᴇʀᴀᴛᴇᴅ ʏᴏᴜʀ ᴛᴏᴋᴇɴ :\n<code>{fallen}</code>\n\n<b>ᴅᴏɴ'ᴛ sʜᴀʀᴇ ᴛʜɪs ᴛᴏᴋᴇɴ ᴡɪᴛʜ ᴀɴʏᴏɴᴇ ᴇʟsᴇ.</b>")

@pbot.on_message(filters.command("revoke") & filters.private)
async def rev_token(_, message: Message):
    if not await is_user(message.from_user.id):
        return await message.reply_text(f"ʟᴏᴏᴋs ʟɪᴋᴇ ʏᴏᴜ'ᴠᴇ ɴᴏᴛ ɢᴇɴᴇʀᴀᴛᴇᴅ ᴛᴏᴋᴇɴ ʙᴇғᴏʀᴇ.")
    fallen = await is_user(message.from_user.id)
    await remove_user(message.from_user.id, fallen)
    await message.reply_text(f"<b>ғᴀʟʟᴇɴ ᴄʜᴀᴛʙᴏᴛ ᴀᴘɪ ᴛᴏᴋᴇɴ ʀᴇᴠᴏᴋᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ.</b>\n\nʏᴏᴜ ᴄᴀɴ ɢᴇɴᴇʀᴀᴛᴇ ʏᴏᴜʀ ᴛᴏᴋᴇɴ ᴀɢᴀɪɴ ʙʏ /token")
