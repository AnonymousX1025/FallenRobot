...
class FallenRobotTelegramHandler:
    def __init__(self, d):
        self._dispatcher = d
    ...
    def command(
        ...
        def _command(func):
            try:
                ...
                log.debug(f"[FALLENCMD] Loaded handler {command} for function {func.__name__} in group {group}")
            except TypeError:
                if can_disable:
                    self._dispatcher.add_handler(
                        DisableAbleCommandHandler(command, func, filters=filters, run_async=run_async,
                ...
                log.debug(f"[FALLENCMD] Loaded handler {command} for function {func.__name__}")
    ...
    def message(self, pattern: Optional[BaseFilter] = None, can_disable: bool = True, run_async: bool = True,
        ...
        def _message(func):
            try:
                ...
                log.debug(f"[FALLENMSG] Loaded filter pattern {pattern} for function {func.__name__} in group {group}")
            except TypeError:
                if can_disable:
                    self._dispatcher.add_handler(
                ...
                log.debug(f"[FALLENMSG] Loaded filter pattern {pattern} for function {func.__name__}")
    ...
    def callbackquery(self, pattern: str = None, run_async: bool = True):
        def _callbackquery(func):
            ...
            log.debug(f'[FALLENCALLBACK] Loaded callbackquery handler with pattern {pattern} for function {func.__name__}')
            return func
    ...
    def inlinequery(self, pattern: Optional[str] = None, run_async: bool = True, pass_user_data: bool = True,
        ...
        def _inlinequery(func):
            ...
            log.debug(
                f'[FALLENINLINE] Loaded inlinequery handler with pattern {pattern} for function {func.__name__} | PASSES '
...
fallencmd = FallenRobotTelegramHandler(d).command
fallenmsg = FallenRobotTelegramHandler(d).message
fallencallback = FallenRobotTelegramHandler(d).callbackquery
falleninline = FallenRobotTelegramHandler(d).inlinequery
